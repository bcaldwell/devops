    ...
    def run
      ...
      Tasks.new_task "Bootstrapping master", list_title: "Master to bootstrap" do
        check? do
          @list = parallel_list [master] do |node|
            !Server.remote_check(node, "KUBECONFIG=/etc/kubernetes/admin.conf kubectl cluster-info")
          end
          @list.empty?
        end
        exec do
      ...
      Tasks.new_task "Joining nodes to cluster", list_title: "Nodes to join" do
        ...
        exec do
          ...
          command = "KUBECONFIG=/etc/kubernetes/admin.conf kubeadm token list | grep 'The default bootstrap token generated by' | awk '{print $1;}'"
          join_token = Server.remote_command(master, command)
          ansible.run_playbook(@list, 'kubernetes/node-join', options: { join_token: join_token, master_ip: "#{master['ip']}:6443" })
        end
        list_logger do |node|
          logger.log(node['ip'])
        end
